...
app.get("/DownloadFile/:id", function(req, res) {
	...
	if (fs.existsSync(__dirname + "/Downloaded/" + req.params.id + ".zip")) {
		...
		res.header("Content-Type", "application/json");
		res.status(404).send({success: false, error_code: 404, message: "File not found."});
	}
});
...
app.get("/Download/:id", function(req, resp) {
	...
	var id = req.params.id, json = {};
	if (!fs.existsSync(__dirname + "/Downloaded/" + id + ".zip")) {
		if (status[id]) return resp.status(400).send({success: false, error_code: 400, message: "This comic chapter is currently being downloaded by another user. Please wait for a few minutes."});
